...
contract dearMoonX is Context, IERC20, Ownable {
    ...
    string private _name = "dearMoonX";
...
// dMX Minter with dMX in the contracts.
contract dMXMinter is Ownable, ReentrancyGuard {
    // The dMX TOKEN!
    IBEP20 public dmx;
    ...
    constructor(IBEP20 _dmx) public {
        dmx = _dmx;
    ...
    function transferOperator(address newOperator) external onlyOperator {
        require(
            ...
            "dMXMinter::transferOperator: new operator is the zero address"
        );
...
// File: contracts\dMXMasterChef.sol
...
contract dMXMasterChef is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        require(_dmxPerBlock <= MAXIMUM_EMISSON_RATE, "dMXMasterChef::constructor: more than maximum emission rate");
    ...
    modifier nonDuplicated(IERC20 _lpToken) {
        require(poolExistence[_lpToken] == false, "dMXMasterChef::nonDuplicated: duplicated");
        _;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 dMXReward = multiplier
            ...
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
        // No need to mint tokens because tokens will be transferred from the minter contract manually
        // envi.mint(devAddress, dMXReward.div(10));
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "dMXMasterChef::withdraw: not good(user balance not enough)");
        require(pool.lpSupply >= _amount, "dMXMasterChef::withdraw: not good(pool balance not enough)");
        updatePool(_pid);
        payOrLockupPendingdMX(_pid);
        if (_amount > 0) {
    ...
    function setDevAddress(address _devAddress) public {
        require(msg.sender == devAddress, "dMXMasterChef::setDevAddress: FORBIDDEN");
        require(_devAddress != address(0), "dMXMasterChef::setDevAddress: ZERO");
        devAddress = _devAddress;
        emit SetDevAddress(msg.sender, _devAddress);
    }

    function setFeeAddress(address _feeAddress) public {
        require(msg.sender == feeAddress, "dMXMasterChef::setFeeAddress: FORBIDDEN");
        require(_feeAddress != address(0), "dMXMasterChef::setFeeAddress: ZERO");
    ...
    function updateEmissionRate(uint256 _dmxPerBlock) public onlyOwner {
        require(_dmxPerBlock <= MAXIMUM_EMISSON_RATE, "dMXMasterChef::updateEmissionRate: too high");
        massUpdatePools();
